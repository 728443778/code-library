...
static inline char* hex2str(const unsigned char* hex, int len, char* str)
{
    int i;
    for(i=0; i<len; i++)
    {
...
static inline char* mkmd5_for_string(const char* buf, char* output)
{
    unsigned char dig[16];
    MDString(buf, dig);
    hex2str(dig, 16, output);
    return output;
}
